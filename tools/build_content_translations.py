...
titles={line.split('|')[0]:line.split('|')[1:] for line in (root/'tools/content_titles.txt').read_text(encoding='utf-8').splitlines() if line}
steps=list(dict.fromkeys(s for r in recipes for s in r['steps']))
step_text={row['source']:row for row in read(root/'tools/content_steps.json')}
assert set(step_text)==set(steps), 'Update translations when cooking steps change'
lesson_text=read(root/'tools/content_lessons.json')
