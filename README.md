...
- `build_*.py` — генерация каталога и переводов; `content_*` — исходные тексты переводов.
...
Строки интерфейса находятся в `strings.xml`, списки выбора — в XML-ресурсах values. Содержимое каталога и уроков — в JSON. Для расширения исходного каталога редактируйте `project/build_catalog.py`, затем запускайте его. При обновлении уже установленного каталога потребуется отдельная миграция версии наполнения в LibraryStore: простая замена JSON не перезаписывает существующую базу.
