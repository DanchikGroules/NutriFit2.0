...
Gotowe pliki: delivery/NutriFit-2.1-debug.apk oraz delivery/NutriFit-AndroidStudio.zip. APK jest podpisany kluczem debugowym i przeznaczony do demonstracji szkolnej. ZIP nie zawiera pamięci podręcznych kompilacji ani lokalnych ścieżek do SDK.
...
build_*.py — generowanie katalogu i tłumaczeń; content_* — źródłowe teksty tłumaczeń.
...
Teksty interfejsu znajdują się w strings.xml, a listy wyboru — w zasobach XML w folderze values. Zawartość katalogu i lekcji znajduje się w plikach JSON. Aby rozszerzyć katalog źródłowy, edytuj project/build_catalog.py, a następnie uruchom go. Przy aktualizacji katalogu w już zainstalowanej aplikacji wymagana jest osobna migracja wersji danych w LibraryStore: zwykła podmiana pliku JSON nie nadpisuje istniejącej bazy danych.
...
Aplikacja nie posiada uprawnienia INTERNET. Zewnętrzne filmy są otwierane po kliknięciu w przeglądarce lub aplikacji platformy wideo i wymagają dostępu do internetu. Wbudowane lekcje i katalog są dostępne offline. Lokalne wideo jest dostępne, dopóki wybrany plik istnieje, a system udostępnia do niego dostęp.

Jedno konto na instalację. Usunięcie aplikacji usuwa jej dane; kopie zapasowe w chmurze oraz przenoszenie danych są wyłączone. Istniejące konta mogą utworzyć kod odzyskiwania w profilu po wprowadzeniu aktualnego hasła.
...
Szczegóły: architektura, źródła, testy, scenariusz prezentacji.

Szczegóły dotyczące tłumaczeń, optymalizacji i animacji: LOCALIZATION.md.
