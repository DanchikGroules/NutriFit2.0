...
public class ProfileStore {
  ...
  public ProfileStore(Context c) {
    prefs = c.getSharedPreferences("profile", Context.MODE_PRIVATE);
  }
  ...
  public Profile load() {
    ...
    p.male = prefs.getBoolean("male", true);
    p.activity = prefs.getInt("activity", 0);
    p.goal = prefs.getInt("goal", 1);
  ...
  public void save(Profile p) {
    boolean saved =
        prefs
            ...
            .putInt("age", p.age)
            .putString("height", Double.toString(p.height))
            .putString("weight", Double.toString(p.weight))
            .putString("target", Double.toString(p.targetWeight))
            .putBoolean("male", p.male)
            .putInt("activity", p.activity)
            .putInt("goal", p.goal)
            .putStringSet("likes", p.likes)
            .putStringSet("dislikes", p.dislikes)
            .putStringSet("allergies", p.allergies)
            .putStringSet("restrictions", p.restrictions)
            .commit();
    if (!saved) throw new IllegalStateException("profile_write_failed");
  }
}
